...
function printMsg(){
	...
	echoColor yellow "--------------------------------------------"
	echo ""
	echo -e "\033[1;;35m1* [\033[0m\033[31mv2rayN/Matsuri/nekobox/nekoray\033[0m\033[1;;35m] Run directly using hysteria core: \033[0m"
	echoColor green "Output the client configuration file to: `pwd`/Hys-${remarks}(v2rayN).json (directly download the generated configuration file [recommended] / copy and paste the configuration below to your local computer)"
	echoColor green "Tips: The client only enables http(10809) and socks5(10808) proxies by default! For other methods, please refer to the hysteria document to modify the client config.json yourself"
	echoColor skyBlue "↓***********************************↓↓↓copy↓↓↓*******************************↓"
	cat ./Hys-${remarks}\(v2rayN\).json
	echoColor skyBlue "↑***********************************↑↑↑copy↑↑↑*******************************↑\n"
	url=`cat /etc/hihy/result/url.txt`
	echo -e "\033[1;;35m2* [\033[0m\033[31mShadowrocket/Matsuri/nekobox/Passwall\033[0m\033[1;;35m] One-click link: \033[0m"
	echoColor green ${url}
	echo -e "\n"
	echo -e "\033[1;;35m3* [\033[0m\033[31mClash.Meta\033[0m\033[1;;35m] The configuration file is already in `pwd`/Hys-${remarks}( clashMeta).yaml output, please download it to the client for use (beta)\033[0m"
	echoColor yellow "--------------------------------------------"
}
...
function hihy(){
	if [ ! -f "/usr/bin/hihy" ]; then
		...
		chmod +x /usr/bin/hihi
	fi	
}

function changeIp64(){
    if [ ! -f "/etc/hihy/conf/hihyServer.json" ]; then
  		echoColor red "hihy was not installed properly!"
        exit
	fi 
	now=`cat /etc/hihy/conf/hihyServer.json | grep "resolve_preference"`
    case ${now} in 
		*"64"*)
			echoColor purple "The current IPv6 priority"
            ...
            if [ -z "${input}" ];then
                ...
                systemctl restart haha
                echoColor green "Done.Ipv4 first now."
            fi
            
		;;
		*"46"*)
			echoColor purple "Current ipv4 takes priority"
...
function getPortBindMsg(){
        ...
        if [ "${msg}" == "" ];then
				...
				command=`echo ${msg} | awk '{print $1}'`
          		...
          		echoColor purple "Port: ${1}/${2} has been occupied by ${command}(${name}), and the process pid is: ${pid}."
  				echoColor green "Should the port usage be automatically closed? (y/N)"
				read bindP
				if [ -z "${bindP}" ];then
					echoColor red "Exit the installation because the port is occupied. Please close or change the port manually..."
					if [ "${1}" == "TCP" ] && [ "${2}" == "80" ];then
						echoColor "If the ${1}/${2} port cannot be closed on demand, please use other certificate acquisition methods"
					fi
					exit
				elif [ "${bindP}" == "y" ] ||  [ "${bindP}" == "Y" ];then
					kill -9 ${pid}
					echoColor purple "Unbinding..."
					sleep 3
					if [ $1 == "TCP" ]; then
						msg=`lsof -i ${1}:${2} | grep LISTEN`
					else
        			...
        			if [ "${msg}" != "" ];then
						echoColor red "Failed to close the port occupation and restart the process after forcibly killing the process. Please check whether there is a daemon process..."
						exit
					else
						echoColor green "Port unbinding successful..."
					fi
				else
					echoColor red "Exit the installation because the port is occupied. Please close or change the port manually..."
					if [ "${1}" == "TCP" ] && [ "${2}" == "80" ];then
						echoColor "If the ${1}/${2} port cannot be closed according to requirements, please use other certificate acquisition methods"
					fi
					exit
				fi
        fi
...
function setHysteriaConfig(){
	...
	echo -e "\033[32mPlease select the certificate application method:\n\n\033[0m\033[33m\033[01m1. Use ACME to apply (recommended, tcp 80/443 needs to be turned on)\n2. Use local certificate File\n3. Self-signed certificate\033[0m\033[32m\n\nEnter the serial number:\033[0m"
    read certNum
	useAcme=false
	useLocalCert=false
	if [ -z "${certNum}" ] || [ "${certNum}" == "3" ];then
		echo -e "\nNote: Self-signed certificates have been subject to a large number of random blocks in the recent period without obfs"
		echoColor red "If you must use a self-signed certificate, please configure obfs obfuscation verification in the configuration below to ensure security."
		echoColor green "Please enter the domain name of the self-signed certificate (default: wechat.com):"
		read domain
		if [ -z "${domain}" ];then
			domain="wechat.com"
		fi
		echo -e "->The domain name of the self-signed certificate is: "`echoColor red ${domain}`"\n"
		ip=`curl -4 -s -m 8 ip.sb`
		if [ -z "${ip}" ];then
			ip=`curl -s -m 8 ip.sb`
		fi
		echoColor green "Determine whether the address used by the client to connect is correct? Public network ip:"`echoColor red ${ip}`"\n"
		while true
		do	
			echo -e "\033[32mPlease select:\n\n\033[0m\033[33m\033[01m1, correct (default)\n2, incorrect, manually enter ip\033[0m\033[32m\ n\nInput serial number:\033[0m"
			read ipNum
			if [ -z "${ipNum}" ] || [ "${ipNum}" == "1" ];then
				break
			elif [ "${ipNum}" == "2" ];then
				echoColor green "Please enter the correct public network ip (no need to add [] to the ipv6 address):"
				read ip
				if [ -z "${ip}" ];then
					echoColor red "Input error, please re-enter..."
					continue
				fi
				break
			else
		...
		echoColor green "Please enter the certificate cert file path (fullchain cert is required and the complete certificate chain is provided):"
		read cert
		while :
		do
			if [ ! -f "${cert}" ];then
				echoColor red "\n\n->The path does not exist, please re-enter!"
				echoColor green "Please enter the certificate cert file path:"
				read  cert
			else
				break
			fi
    	...
    	echoColor green "Please enter the domain name (needs to be correctly resolved to the local machine, turn off CDN):"
		read domain
		while :
		do
			if [ -z "${domain}" ];then
		...
		do	
			...
			if [ -z "${ip_resolv}" ];then
				echoColor red "\n\n->Domain name resolution failed, no dns record (A/AAAA) was obtained, please check whether the domain name is correctly resolved to this machine!"
				echoColor green "Please enter the domain name (needs to be correctly resolved to the local machine, turn off CDN):"
				read  domain
				continue
			fi
			...
			if [ -z "${localip}" ];then
				localip=`curl -s -m 8 ip.sb` #If the above ip.sb fails, check it one last time
				if [ -z "${localip}" ];then
					echoColor red "\n\n->Failed to obtain local IP, please check the network connection! curl -s -m 8 ip.sb"
					exit 1
				fi
			fi
			if [ "${localip}" != "${remoteip}" ];then
				echo -e " \n\n->Local IP: "`echoColor red ${localip}`" \n\n->Domain name IP: "`echoColor red ${remoteip}`"\n"
				echoColor green "The detection may fail when multiple IPs or DNS are not effective. If you are sure that the local machine is correctly resolved, do you want to specify the local IP yourself? [y/N]:"
				read isLocalip
				if [ "${isLocalip}" == "y" ];then
					echoColor green "Please enter your local IP:"
					read localip
					while :
					do
						if [ -z "${localip}" ];then
    ...
    echo -e "\n->Transport protocol:"`echoColor red ${protocol}`"\n"

	while :
	do
		echoColor green "Please enter the port you want to open. This port is the server port. It is recommended to be 10000-65535. (Default is random)"
		read  port
		if [ -z "${port}" ];then
		...
		if [ "${port}" -gt 65535 ];then
			echoColor red "Wrong port range, please re-enter!"
			continue
		fi
		if [ "${ut}" != "udp" ];then
	...
	if [ "${protocol}" == "udp" ];then
		echoColor green "\n->It is detected that you have selected the udp protocol, you can use the [Port Hopping/Multi-port] (Port Hopping) function, it is recommended to use"
		echo -e "Tip: Long-term single-port UDP connections are easily blocked/QoS/interrupted by the operator. Enabling this function can effectively avoid this problem."
		...
		if [ -z "${portHoppingStatus}" ] || [ $portHoppingStatus == "1" ];then
			...
			echoColor purple "\n->You choose to enable the port hopping/multi-port (Port Hopping) function"
			echo -e "Port hopping/multi-port (Port Hopping) function needs to occupy multiple ports. Please ensure that these ports are not listening to other services\nTip: The number of port selections should not be too many. It is recommended to choose around 1000, ranging from 1-65535. It is recommended to choose Contiguous port range.\n"
			while :
			do
				...
				if [ $portHoppingStart -gt 65535 ];then
					echoColor red "\n->Wrong port range, please re-enter!"
					continue
				fi
				echo -e "\n->Starting port:"`echoColor red ${portHoppingStart}`"\n"
				...
				if [ $portHoppingStart -ge $portHoppingEnd ];then
					echoColor red "\n->The starting port must be smaller than the ending port, please re-enter!"
				else
					break
				fi
			done
			...
			echoColor red "\n->You choose to skip the port hopping/multi-port (Port Hopping) function"
		fi
	fi
    ...
    echo -e "\nExpected speed, this is the peak speed of the client, the server is not limited by default."`echoColor red Tips: The script will automatically *1.10 do redundancy. If your expectation is too low or too high, it will affect the forwarding efficiency. Please fill it in truthfully!`
    ...
    echo -e "\033[32m Please enter the uplink speed expected by the client (default 10, unit: mbps):\033[0m"
    read  upload
    if [ -z "${upload}" ];then
        upload=10
    fi
	...
	auth_str=""
	obfs_str=""
	echo -e "Tips: If you use obfs obfuscation encryption, the anti-blocking ability is stronger and can be identified as unknown UDP traffic, but it will increase the CPU load and cause the peak speed to drop. If you are pursuing performance and are not blocked, it is not recommended."
	echo -e "\033[32mSelect authentication method:\n\n\033[0m\033[33m\033[01m1, auth_str (default)\n2, obfs\033[0m\033[32m\n\nEnter Serial number:\033[0m"
	read auth_num
...
EOF
	...
	echo -e "\n->The verification method you selected is:"`echoColor red ${auth_type}`"\n"
	echoColor green "Please enter the client name remarks (default domain name/IP distinction, for example, enter test, the name is Hys-test):"
	read remarks
...
EOF
	...
	case ${msg} in 
		*"Failed to get a certificate with ACME"*)
			echoColor red "Domain name: ${u_host}, certificate application failed! Please check whether the panel firewall provided by the server is turned on (TCP:80,443)\nWhether the domain name is correctly resolved to this IP (do not open CDN!)\nIf the above two cannot be met Click, please reinstall and use the self-signed certificate."
	...
	echoColor greenWhite "Installation successful, please check the configuration details below"
}
...
function downloadHysteriaCore(){
	...
	#Compatible with v1 version updates after v2 is released (temporary, removed in the next version)
	version="v1.3.5"
...
function updateHysteriaCore(){
	if [ -f "/etc/hihy/bin/appS" ]; then
		...
		if [ "${localV}" = "${remoteV}" ];then
			...
			if [ "${status}" = "active" ];then #If it is running normally, the daemon will be stopped first and then automatically updated and then restarted, otherwise it will only be responsible for updating.
				systemctl stop hihi
				downloadHysteriaCore
				systemctl start hihi
			else
				downloadHysteriaCore
			fi
...
function hyCoreNotify(){
	if [ -f "/etc/hihy/bin/appS" ]; then
		...
		if [ -z $remoteV ];then
			...
			if [ "${localV}" != "${remoteV}" ];then
				echoColor purple "[Update] hysteria has an update, version:${remoteV}. Log: https://github.com/apernet/hysteria/blob/master/CHANGELOG.md"
			fi
		fi
...
function checkStatus(){
    ...
    if [ "${status}" = "active" ];then
		...
		echoColor red "Dead!hysteria is not running properly!"
	fi
}
...
function addPortHoppingNat() {
	...
	# $2 portHoppingEnd
	# $3 portHoppingTarget
	# If the firewall is enabled, delete the previous rules
	if [[ -n $(find /etc -name "redhat-release") ]] || grep </proc/version -q -i "centos"; then
		mkdir -p /etc/yum.repos.d
	...
	if systemctl status firewalld 2>/dev/null | grep -q "active (running)"; then
		...
		if ! [ -x "$(command -v netfilter-persistent)" ]; then
			echoColor red "[Warning]: The installation of netfilter-persistent failed, but the installation progress will not stop. It is just that your PortHopping forwarding rules are temporary rules and may become invalid after restarting. Do you want to continue using the temporary rules? (y/N)"
		...
		if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
			...
			echoColor red "netfilter-persistent is not started. PortHopping forwarding rules cannot be persisted. Restarting the system will fail. Please manually execute netfilter-persistent save. Continuing to execute the script will not affect subsequent configurations..."
		fi
...
function changeMode(){
	...
	echoColor yellow "The current protocol used is:"
	echoColor purple "${protocol}"
	port=`cat /etc/hihy/conf/hihyServer.json | grep "listen" | awk '{print $2}' | tr -cd "[0-9]"`
	if [ "${protocol}" = "udp" ];then
		echo -e "\033[32m\nPlease select the protocol type to modify:\n\n\033[0m\033[33m\033[01m1, faketcp\n2, wechat-video\033[0m\033[32m\ n\nInput serial number:\033[0m"
    	read pNum
		if [ -z "${pNum}" ] || [ "${pNum}" == "1" ];then
			echoColor purple "Choose to modify the protocol type to faketcp."
		...
		echoColor red "Unable to recognize protocol type!"
		exit
	fi
	systemctl restart haha
...
function cronTask(){
	systemctl restart hihy #Prevent hysteria from occupying too much memory
	systemctl restart systemd-journald #Prevent the log from occupying too much memory
}
...
function start(){
	...
	if [ "${status}" = "active" ];then
		...
		echoColor red "Check the error log and report it to the issue!"
	fi
}
...
|**********     Version: `echoColor red "${hihyV}"`    **********|
 -------------------------------------------
Tips: `echoColor green "hihy"` command is used to run this script again.
`echoColor skyBlue "............................................."`
`echoColor purple "###############################"`
...
if [ "$1" == "install" ] || [ "$1" == "1" ]; then
	...
	echoColor purple "-> 4) Pause hysteria"
	stop
	...
	echoColor purple "-> 8) View current configuration"
	printMsg
elif [ "$1" == "changeServerConfig" ]  || [ "$1" == "9" ]; then
	echoColor purple "-> 9) Reconfigure"
	changeServerConfig
	...
	echoColor purple "-> 14) View real-time log"
	checkLogs
